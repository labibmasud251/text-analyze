...
export const countWords = async(text: string) => {
    let words = text.split(/\s+/).filter(Boolean);
    let wordCount = words.length;
    return wordCount;
}

export const countCharacters = async(text: string) => {
    let characters = text.split('');
    let characterCount = characters.length;
    return characterCount;
}
export const countSentences = async(text: string) => {
    let sentences = text.split(/[.!?]/).filter(Boolean);
    let sentenceCount = sentences.length;
    return sentenceCount;
}
export const countParagraphs = async(text: string) => {
    let paragraphs = text.split(/\n\s*\n/).filter(Boolean);
    let paragraphCount = paragraphs.length;
    return paragraphCount;
}
export const longestWordsInParagraphs = async(text: string) => {
    let paragraphs = text.split(/\n\s*\n/).filter(Boolean);
    let longestWords = [];
